...
- 非机密域：MSI 地址保持不变。
- 机密域：MSI 地址增加 `0x80`，也就是在原 interrupt file 页内增加半页偏移。
...
interrupt source
  ...
  -> APLIC adds 0x80 to the MSI address for confidential sources
...
因为 IMSIC 内部同时维护机密域和非机密域的 S/VS interrupt files，输出到 CSR 侧的 pending 信息也需要分成两份。

`pendings` 需要分别输出非机密域和机密域的 pending bitmap，避免两个域在同一组 CSR 可见 pending 状态中混合。`topeis` 也需要分别输出两份，分别表示两个域内当前最高优先级的 pending interrupt。
